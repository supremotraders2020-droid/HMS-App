...
export default function HMSSidebar({ currentRole, currentUser, onNavigate, onLogout }: HMSSidebarProps) {
  ...
  const getMenuItems = (role: UserRole) => {
    ...
    const services = [
      ...
      { title: "IPD Monitoring", url: "/patient-monitoring", icon: Stethoscope, roles: ["ADMIN", "DOCTOR", "NURSE"] },
      { title: "ICU Monitoring", url: "/icu-monitoring", icon: Heart, roles: ["SUPER_ADMIN", "ADMIN", "DOCTOR", "NURSE"] },
      { title: "Operation & OT", url: "/operation-ot", icon: Scissors, roles: ["SUPER_ADMIN", "ADMIN", "DOCTOR", "NURSE"] },
      ...
      { title: "Referral Data", url: "/referral-data", icon: UserPlus, roles: ["ADMIN"] },
          ...
          <SidebarGroupContent>
            <SidebarMenu className="space-y-1">
              {getMenuItems(currentRole).filter(item => 
                ['OPD Service', 'Patient Service', 'Patient Tracking', 'IPD Monitoring', 'ICU Monitoring', 'Operation & OT', 'Biometric Service', 'Prescriptions', 'Staff Management', 'Services & Surgeries'].includes(item.title)
              ).map((item, index) => (
        ...
        {currentRole !== "OPD_MANAGER" && currentRole !== "NURSE" && currentRole !== "SUPER_ADMIN" && currentRole !== "TECHNICIAN" && (
          <SidebarGroup>
            ...
            <SidebarGroupContent>
              <SidebarMenu className="space-y-1">
                {getMenuItems(currentRole).filter(item => 
                  !['Dashboard', 'OPD Service', 'Patient Service', 'Inventory Service', 'Patient Tracking', 'Biometric Service', 'IPD Monitoring', 'ICU Monitoring', 'Operation & OT', 'Prescriptions', 'Equipment Servicing', 'Oxygen Tracker', 'Chatbot Service', 'Notification Service', 'Staff Management', 'Services & Surgeries'].includes(item.title)
                ).map((item, index) => (
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton 
                      asChild
