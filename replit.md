...
- **Super Admin Portal**: Enterprise-level control center with 11 fully-functional sections: Dashboard (system metrics), User Roles & Permissions (permission matrix with CRUD controls), Billing Finalization (financial locking workflows), Stock & Pharmacy Control (batch management, expiry tracking), Surgery Packages (package pricing with OT/surgeon/anesthesia fees), Medicine Database (salt composition, dosage forms, MRP management), Insurance Providers (TPA integration, coverage limits), Claims Management (approval workflows, rejection reasons), Hospital Packages (health checkup packages), Audit Logs (immutable action trail with 50+ action types), and System Settings (hospital configuration). Implements hierarchical RBAC with SUPER_ADMIN as highest authority, NABH/HIPAA compliance with encrypted sensitive data, and complete audit traceability. Available exclusively to SUPER_ADMIN role.
